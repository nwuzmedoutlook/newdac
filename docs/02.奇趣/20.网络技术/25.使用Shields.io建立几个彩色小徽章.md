...
```
其中"status":200表示数据请求成功，"totalSubs":150表示请求到的总粉丝量，"subsInEachSource"包含每个服务请求到的数据量（😎其实是可以同时请求多个服务的哦），我们这里只有"bilibili":150，"failedSources"表示请求失败的数据源，暂无。
...
如果想在前面加上logo，只需要在链接最后加上<mark>&logo=bilibili</mark>即可
