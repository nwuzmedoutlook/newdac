---
title: 关于我
date: 2019-12-25 14:27:01
permalink: /me/
sidebar: false
article: false
author:
  name: NWUzmed
  link: https://github.com/nwuzmedoutlook/
---

## 个人站点

<ClientOnly>
  <Card :cardData="cardData0" :cardListSize=4 carTitlColor="#000" carHoverColor="#000" />
</ClientOnly>

## 社交平台

<a href="https://github.com/" target="_blank">
  <img src="https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.spencerwoo.com%2Fsubstats%2F%3Fsource%3Dgithub%26queryKey%3Dnwuzmedoutlook&label=Github&query=$.data.totalSubs&color=fff&logo=github&labelColor=grey" height="80">
</a>&emsp;
<a href="https://www.bilibili.com/" target="_blank">
  <img src="https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.spencerwoo.com%2Fsubstats%2F%3Fsource%3Dbilibili%26queryKey%3D327638134&label=Bilibili&query=$.data.totalSubs&color=blue&logo=bilibili&labelColor=FE7398" height="80">
</a>

<br>

<a href="https://music.163.com/" target="_blank">
  <img src="https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.spencerwoo.com%2Fsubstats%2F%3Fsource%3DneteaseMusic%26queryKey%3D1326518199&label=网易云音乐&query=$.data.totalSubs&color=282c34&logo=data:image/svg+xml;base64,&labelColor=e72d2c" height="80">
</a>&emsp;
<a href="https://www.weibo.com/" target="_blank">
  <img src="https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.spencerwoo.com%2Fsubstats%2F%3Fsource%3Dweibo%26queryKey%3D3525494132&label=微博&query=$.data.totalSubs&color=fdb933&logo=sina-weibo&labelColor=e71f19" height="80">
</a>

<br>

<a href="https://www.zhihu.com/" target="_blank">
  <img src="https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.spencerwoo.com%2Fsubstats%2F%3Fsource%3Dzhihu%26queryKey%3DNWUzmed&label=知乎&query=$.data.totalSubs&color=blue&logo=zhihu&labelColor=white" height="80">
</a>&emsp;
<a href="https://www.facebook.com/" target="_blank">
  <img src="https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.spencerwoo.com%2Fsubstats%2F%3Fsource%3Dfacebook%26queryKey%3D100024409567304&label=Facebook&query=$.data.totalSubs&color=blue&logo=facebook&labelColor=white" height="80">
</a>

<br>

<a href="https://twitter.com/" target="_blank">
  <img src="https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.spencerwoo.com%2Fsubstats%2F%3Fsource%3Dtwitter%26queryKey%3DNWUzmed&label=Twitter&query=$.data.totalSubs&color=blue&logo=twitter&labelColor=white" height="80">
</a>

## 联系方式
- QQ: <a href="http://sighttp.qq.com/authd?IDKEY=96dd73b876713d27bc33f9e16e4fb9335b1f5f33d5e7fa18" target="_blank">459208581</a>
- Email: nwuzmed@outlook.com
- GitHub: <https://github.com/nwuzmedoutlook/>
- 交流QQ群：<https://qq-group.cf/>

<script>
export default {
  data() {
    return {
      cardData0: [
{id: "0", cardSrc: "https://co2co2.cf/", cardImgSrc: "https://api.xinac.net/icon/?url=https://co2co2.cf/", cardName: "NWUzmed's world", cardContent: "原第2个博客，主题hexo-theme-Material X",},
{cardSrc: "http://nwuzmed.cf/", cardImgSrc: "https://api.xinac.net/icon/?url=http://nwuzmed.cf/", cardName: "子穆二铎", cardContent: "原第1个博客，主题hexo-theme-matery",},
{cardSrc: "https://goooogle.ga/", cardImgSrc: "https://api.xinac.net/icon/?url=https://goooogle.ga/", cardName: "Nation个人主页", cardContent: "基于Nation+Github+Vercel+Freenom搭建",},
{cardSrc: "https://ccus.cf/", cardImgSrc: "https://api.xinac.net/icon/?url=https://ccus.cf/", cardName: "NWUzmed | Resume", cardContent: "个人简历",},
{cardSrc: "https://www.ctrl-f.cf/", cardImgSrc: "https://api.xinac.net/icon/?url=https://www.ctrl-f.cf/", cardName: "Zmed", cardContent: "基于Railway+Github+Vercel+Freenom+Typecho搭建",},
{cardSrc: "https://mark-down.cf/", cardImgSrc: "https://api.xinac.net/icon/?url=https://mark-down.cf/", cardName: "Waline评论系统", cardContent: "基于Leancloud+Vercel+Github",},
{cardSrc: "https://github.com/nwuzmedoutlook/cors-server", cardImgSrc: "https://api.xinac.net/icon/?url=https://github.com/nwuzmedoutlook/cors-server", cardName: "Gitalk跨域代理", cardContent: "基于Netlify+GitHUb搭建",},
{cardSrc: "https://github.com/nwuzmedoutlook/twikoo", cardImgSrc: "https://api.xinac.net/icon/?url=https://github.com/nwuzmedoutlook/twikoo", cardName: "Twikoo评论系统", cardContent: "基于MongoDB+Vercel+Github+Freenom搭建",},
      ],
    };
  },
};
</script>
